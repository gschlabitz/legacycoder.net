...
- **`@strudel/repl` and `@strudel/web` do not share module instances.** The
  ...
  is why the lab leaves for a subdomain (issue #26).
...
1. **Spike the resume mechanics** in the existing `StrudelPlayer` island
   before any packaging: confirm `scheduler.now()` → `Math.ceil` →
   `.early()` → `setPattern()` produces a clean downbeat re-entry, and find
   where a master gain node attaches for the ramp.
