...
- `./morph sweep` keeps the latest ready warm snapshot and deletes
  superseded warm snapshots, any hot-dev snapshots (they carry
  secrets), and all purpose-less snapshots (build debris). Never sweep
  while a warm build is running.
...
- Never put secrets in `morph:warm`'s snapshot layers or snapshot a task
  instance — task disks carry per-run credentials. Hot boxes carry the
  same per-run secrets on a paused disk, deliberately longer-lived;
  never snapshot one.
